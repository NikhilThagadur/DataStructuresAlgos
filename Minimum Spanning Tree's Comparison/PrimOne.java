...
//    	g.PSTMain(g);
        ...
        System.out.println("Prim Implementation One....Lazy Version");
...
class AdjList
{
	...
	public AdjListNode newAdjNode(int source ,int destination, int weight){
...
class Graph
{
    ...
    public void addEdge(Graph gph, int source, int destination, int weight){
    	...
    	AdjListNode adn =  al.newAdjNode(source,destination, weight);
    	
    	adn.next = gph.array[source].head;
    	gph.array[source].head = adn;    	
    	adn =  al.newAdjNode(destination,source, weight);
    	adn.next = gph.array[destination].head;
    	gph.array[destination].head = adn;
    }
    
    public void visit(Graph gph, minHeap hp, int v){
    	marked[v] = true;
    	AdjListNode p = gph.array[v].head;
    	while(p!=null){
    		int dest = p.dest;
    		if(marked[dest]==false){
    			minHeapNode  mhn = hp.newMinHeapNode(dest, p.weight);
    			hp.displayHeap(hp);
    			hp.insert(hp, mhn);
    			//System.out.println(" dest " + dest + " weight " + p.weight + " is added to heap");
    		//	System.out.println("____________ heap size " + hp.size + " ____________________");
    			hp.displayHeap(hp);
    ...
    public void PSTMain(Graph gph){
    	...
    	int [] key =new int[V];
    	int MST=0;
    	//System.out.println(" size " + gph.V);
    	minHeap hp = new minHeap();
    	hp.createMinHeap(V);
    	hp.array = new minHeapNode[V*2];
    	hp.pos = new int[V];
//    	for(int i =1;i<V;i++){
//    		parent[i]=-1;
//    		key[i]=1000000000;
...
//    	key[0]=0;
    	hp.insert(hp, hp.newMinHeapNode(0, 0));
    	//hp.array[0]=0;
    	hp.pos[0]=hp.posCounter;
    	hp.posCounter++;
//    	hp.size = V;
//    	hp.displayHeap();
